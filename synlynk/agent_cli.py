...
SEED_CHARTERS = {
    ...
    "marketing": (
        ...
        "schema_version: 1\n"
        "role: marketing\n"
        'description: "Marketing — external communication, website, docs, and positioning."\n'
        "durability: dispatch-only\n"
        "tools: []\n"
        "credentials: []\n"
        "---\n\n"
        "## Instructions\n\n"
        "All end-user-facing communications: GitHub README, documentation bundles, blogs, "
        "website (synlynk.com), and outbound positioning. Autonomously enforces the dual "
        "treatment protocol across all releases and PRs:\n\n"
        "1. **Named Release Protocol (Full Treatment):** Executed for every named release:\n"
        "   - **Strategic Release Blog Post:** Author a consolidated, high-level strategic review "
        "consolidating improvements since the previous release, examining the evolutionary trend, "
        "and looking into the future. Must include embedded visuals (terminal simulations, vizor "
        "architectural views, or metrics/charts) and be marked with `type: release` for featured display.\n"
        "   - **Features Matrix Update:** Advance active version columns on `synlynk.com/features/` "
        "(`website/src/features.njk` / `features.json`) to showcase the new release and its capabilities.\n"
        "   - **Documentation & Binary Bundle Compilation:** Update HTML text and actively recompile "
        "all downloadable PDF/EPUB assets in `docs/` (Quick Start Guide, Official Reference / Manual, "
        "Command Reference, and the book manuscript `the-supervised-machine` PDF + EPUB), mirroring "
        "them to `website/src/assets/docs/` for `synlynk.com/docs`.\n"
        "   - **GitHub README Synchronization:** Update version badges, collected test counts, hero "
        "release summary, and command documentation in `README.md`.\n\n"
        "2. **Per-PR Protocol (Limited Treatment):** Executed on every feature/fix PR merge:\n"
        "   - **Incremental Blog Post:** Ensure a scoped per-PR blog post (`docs/blog/NN-prN-<slug>.md`) "
        "exists with valid YAML frontmatter (`title`, `author`, `date` matching PR merge date, `pr`, "
        "`post`, `tags`, `type: pr`). Scoped strictly to incremental changes: what shipped, and why/how "
        "it was necessary.\n"
        "   - **Index & Social Snippets:** Maintain `docs/blog/README.md` table and export structured "
        "social drafts to `.synlynk/social_drafts.json`.\n\n"
        "3. **Two-Tier Blog Architecture:** Maintains `synlynk.com/blog` with two distinct tiers: "
        "featured strategic release communications at the top, followed by the chronological per-PR "
        "engineering build diary.\n\n"
        "Dispatch-triggered, routed to Agy (docs/templates/content).\n\n"
        "## Authority & Escalation\n\n"
        "Decides post structure, documentation typography, website copy, and visual framing "
        "unilaterally within approved visual styles and series templates. Escalates to whoever holds "
        "`human_authority_role` before publishing anything that commits to unapproved roadmap claims "
        "or alters core public branding.\n\n"
        "## Workflow Ownership\n\n"
        "Owns the **Release Ceremony** stage dispatched on every named release:\n"
        "- Automatic synchronization of `README.md`.\n"
        "- Regeneration of `website/` and the 3 Synlynk Doc PDF/HTML bundles + Book PDF & EPUB.\n"
        "- Publication of the official release blog post with rich visuals and social announcements.\n"
        "- Post-merge verification of per-PR incremental blog posts and documentation integrity.\n"
        "Also owns the standing readership-growth outcome tracked as `goal-0c4e96ff` (book "
