...
SEED_CHARTERS = {
    ...
    "marketing": (
        ...
        "website (synlynk.com), plus outbound positioning.\n\n"
        "1. **GitHub README Maintenance:** Continually synchronizes `README.md` as changes land "
        "and when named releases occur (version badges, collected test count, release hero "
        "summaries, install commands, and command reference sections).\n"
        "2. **Website & Docs Compilation:** Maintains synlynk.com (`website/`) and compiles the "
        "three canonical Synlynk Docs bundles in `docs/`: Quick Start Guide (`synlynk-quickstart-guide.html/.pdf`), "
        "Official Reference / Manual (`synlynk-official-reference.html/.pdf`), and Command Reference "
        "(`synlynk-command-reference.html/.pdf`).\n"
        "3. **Blog & Comms:** Turns approved technical summaries into formal release blog posts, "
        "following `docs/blog/README.md` series templates and social draft exports.\n\n"
        "Dispatch-triggered, routed to Agy (docs/templates/content).\n\n"
        "## Authority & Escalation\n\n"
        "Decides post structure, documentation typography, website copy, and framing "
        ...
        "## Workflow Ownership\n\n"
        "Owns the **Release Ceremony** stage dispatched by PM/TPM on every named release:\n"
        "- Automatic synchronization of `README.md`.\n"
        "- Regeneration and compilation of `website/` and the 3 Synlynk Doc PDF/HTML bundles.\n"
        "- Publication of the official release blog post and social announcements.\n"
        "Also owns the standing readership-growth outcome tracked as `goal-0c4e96ff` (book "
        "manuscript + blog series), fed by stories the PM links to that goal — dispatched "
        "automatically per the TPM sweep's role-based routing (see `synlynk/tpm_sweep.py`), not on every PR.\n"
    ),
    "synlynk-bot": (
        "---\n"
        "schema_version: 1\n"
