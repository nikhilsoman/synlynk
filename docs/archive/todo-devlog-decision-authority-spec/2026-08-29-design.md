...
**Status:** ARCHIVED (superseded) 2026-08-29 — built on a false premise for 2 of its 3 sections, see below.
**Originally:** Approved by Nikhil 2026-08-29
...
> **Superseded, same day.** This spec's "Mechanism: devlogs" and "Mechanism: decisions" sections propose building `devlog_entries`/`decision_entries` tables and write-through logic that **already exist** — implemented by PR #1038 (`c3951e4`, merged 2026-08-17), which this spec's authors did not verify against current code before writing. #1219, which this spec was meant to resolve, has been closed on that basis (no open authority decision remains for devlogs/decisions). #936, which this spec claimed to resolve root causes #1/#2 for, was re-closed citing PR #1038 as the actual fix.
>
> The "Mechanism: todo.md" section's core idea (regenerate `todo.md` from the `stories` table via the already-existing, already-migration-aware `_generate_todo_md()`, backed by a guarded one-time backfill) remains directionally correct and is **not** invalidated — but it is now tracked directly in the widened description on **#1218**, rather than through this standalone spec. That issue comment also flags the existing `_import_todo_to_stories()` parser (`synlynk/db.py:2207`) as a candidate to reuse for backfill instead of the from-scratch mechanism this spec designed, which this spec's authors were unaware of.
>
> Kept here for reference (the backfill/regeneration/hand-edit-drift design reasoning below may still be useful when #1218 is implemented) but should not be treated as an approved, current spec. Do not implement directly from this document — use #1218's issue description as the authoritative scope.
