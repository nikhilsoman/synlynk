...
| **Codex** | **Unreliable** — sandbox blocks network egress to `api.github.com` by design (#426); confirmed again on PR #1176 (`error connecting to api.github.com`) | Not authorized (same network-sandbox limitation) | Reliable — primary harness for implement/test/refactor/cli-plumbing | Network sandbox is structural, not a bug — don't re-test this one without a sandbox config change |
