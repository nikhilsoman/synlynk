...
**Distribution & linkage**: `synlynk/capability_baseline.json` ships in package releases; `_maybe_open_worktree_pr()` now captures and returns the created/existing PR's number so `capability_ratings.pr_number` can be set at job-finalization time, giving the multiplier something to join against.
