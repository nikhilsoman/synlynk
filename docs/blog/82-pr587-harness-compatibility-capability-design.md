...
- **Codex's approval flag is `--ask-for-approval`** (`untrusted|on-request|never`), not the legacy Codex approval-flag name as `AGENT_CAPABILITY_BASELINES` currently has it.
