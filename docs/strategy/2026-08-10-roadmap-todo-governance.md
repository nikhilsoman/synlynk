...
The devlog ownership boundary in this document's three-layer decomposition is also worth stating explicitly, since it's easy to blur once tpm starts writing to any of these layers: the user/harness session owns the devlog's narrative — intent, reasoning, what happened and why, in the author's own words. tpm may append structured evidence and reconciliation results (attribution, disposition, staleness flags) alongside that narrative, but must never rewrite it, and must never silently promote a harness suggestion or an unreviewed item into a roadmap commitment on the narrative's behalf. tpm's write surface is evidence and bookkeeping, not authorship.
...
1. **Assign ownership explicitly, not just schema.** `state.db` remains the shared canonical store for work envelopes, devlog relationships, goals, stories, and evidence/attribution — that does not change. What's missing is an accountable operator: tpm should be the durable loop that runs the `synlynk checkpoint` reconciliation described above, on its own cadence, not something that depends on a human or harness remembering to invoke it at session end. tpm is the operator of this mechanism, not a new source of truth or a role-private database silo — GOVERNS remains the lifecycle enforcement authority (see item 4) and pm remains the human/escalation authority for ambiguous attribution or promoting something to a new goal (see item 3). That is the actual fix for roadmap and todo falling behind: the missing piece was never a better schema, it was a named, durable, accountable actor whose job is specifically that this doesn't drift — which is exactly what "durable" already means in tpm's charter.
...
Concretely, the first task should extend GOVERNS' existing event contract (PR #817) with the typed events tpm's read model needs, rather than standing up a parallel tpm-only event bus: at minimum `session_opened`, `session_checkpointed`, `session_closed`, `evidence_unattributed`, and `github_issue_untriaged`, plus an issue/PR divergence event for the reconciler described above. These are additions to GOVERNS' contract — GOVERNS still owns lifecycle enforcement; tpm only consumes and derives state from it, per item 4 above.
