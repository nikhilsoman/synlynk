...
- **Mutation guard (the panel's flagged must-have, not optional):** every generated file gets a header comment identical in spirit to `todo.md`'s existing "generated — do not hand-edit" banner, and `synlynk` commands that read these files for context injection or `pr check` detect a hand-edit and warn loudly, printing the drift and continuing rather than halting the command (see §8.2 for the confirmed warn-and-continue landing decision). This is what prevents the exact incident from recurring one layer down, per Claude/Codex/Grok's shared risk callout, without adding friction to every session that touches these files.
  - **What counts as a hand-edit (tier-1 scoped, revised after review):** the guard compares the working-tree file against (a) what regeneration would currently produce from the local DB, and (b) the file's content at the last commit that touched it (`git show HEAD:<path>`). Divergence from (a) alone doesn't trigger a warning — a fresh `git pull`/merge legitimately changes the on-disk file without the local DB having been mutated yet. The guard only warns when the working tree diverges from **both** the last-known-good git blob **and** current regeneration output in a way regeneration can't explain (i.e., uncommitted local edits that don't correspond to any DB state, committed or pending). A file that matches its last git-committed blob but is stale relative to a `git pull` is not a violation — it's an input to resync, not a hand-edit. §7's DoD is updated to require a test for this pull-then-resync path specifically, since it's the case tier 1's original wording would have wrongly flagged.
  - This distinction is what unblocks Tier 2's reconciliation flow (§2.2) without weakening tier 1's guarantee: tier 1 still warns loudly on genuine uncommitted hand-edits (without blocking the command), but doesn't brick every `synlynk` command the instant a teammate's merged change lands via git.
...
- Hand-editing any of the four `project-docs/` files (including `todo.md`, unchanged) is detected and warned about loudly by tooling — not just forbidden by a comment, and not silently ignored — including a test for the revised guard's git-pull-then-resync case (§2.1), not just the pure-hand-edit case.
...
**8.2 Mutation guard ships warn-and-continue, not block-and-refuse.** §2.1's original wording ("refuses to proceed", "the guard only refuses when...") described block-and-halt behavior, which conflicted with this decision — a PR review of this addendum caught the inconsistency. §2.1 and §7 have been reworded above to consistently describe warn-and-continue: the guard detects genuine uncommitted hand-edits not explained by a `git pull`/merge, prints a loud warning on the next `synlynk` invocation, and continues rather than halting the command. This matches the project's existing "fail loud, don't fail closed" pattern (e.g. `cmd_migrate()`'s 0-row import check).
