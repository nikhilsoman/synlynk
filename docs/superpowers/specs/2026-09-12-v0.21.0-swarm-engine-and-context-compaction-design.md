...
```mermaid
graph TD
    subgraph "Milestone v0.21.0 Architecture"
        subgraph "Cluster A: Ephemeral Swarm Execution Infrastructure"
            S1["Pluggable Runner Driver Contract<br>(synlynk/runners/base.py)"]
            S2["Multi-Cloud Runners<br>(Docker · K8s · Fly.io)"]
            S3["Swarm CLI & Relay Sync<br>(synlynk swarm dispatch)"]
        end

        subgraph "Cluster B: Prompt Cache Telemetry True-Up"
            C1["Unified Cache Token Extractor<br>(All 5 Core Harnesses)"]
            C2["Cache-Aware Billing Ledger<br>& Hierarchical Cost Rollup"]
            C3["Real-Time Cache Hit Telemetry<br>(synlynk status · Vizor)"]
        end

        subgraph "Cluster C: Autonomous Context Compaction Engine"
            M1["Continuous Token Budget Monitor<br>(75% Threshold Alerting)"]
            M2["AST-Grounded State Checkpointer<br>& Devlog Rescue Protocol"]
            M3["Safe Memory Compaction Pipeline<br>(synlynk context compact)"]
        end

        subgraph "Cluster D: Living Charter Adaptation Loops"
            A1["Empirical Capability Drift Scoring<br>(telemetry vs charter)"]
            A2["Automated Charter Amendment Generator<br>(docs/charters/proposals/)"]
            A3["1-Click Approval & state.db Promotion<br>(synlynk charters adapt --write)"]
        end
    end
