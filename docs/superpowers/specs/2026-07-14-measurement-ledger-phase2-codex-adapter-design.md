...
2. Codex's job-launch path in `dispatch_agent()` currently builds a shell string and redirects it directly to the log file (`f"{cmd_str} > {log_file} 2>&1"`). With `--json`, that log file would become raw JSONL — unreadable in `synlynk logs --job <id>`, a real UX regression from what every other agent's log looks like today.

   Codex's launch gets its own code path instead of the shared shell-redirect string: spawn `subprocess.Popen(cmd, stdout=PIPE, stderr=STDOUT)` (matching the existing `_tee_process` shape used in `exec_command()`) and run a dedicated rendering thread, `_render_codex_jsonl_to_log(process, log_file)`, that reads stdout line-by-line and writes a human-readable reconstruction to the log file as each event arrives:
   - `item.completed` with `item.type == "agent_message"` → write `item["text"]` followed by a blank line (matches what Codex's own plain-text mode shows for agent messages).
   - `item.completed` with `item.type == "command_execution"` → write `$ {item["command"]}` then `item["aggregated_output"]` (trimmed of trailing whitespace) then a blank line.
   - `item.started` events are not written (only the completed/final state of each item matters for a log transcript — avoids duplicate/partial lines).
   - `turn.completed` → not written to the visible log (it's metadata, not part of the transcript); the raw line is *also* appended to a separate small buffer that the reconciliation step reads for the `usage` object, OR (simpler) the reconciliation step re-derives usage the same way it does for every other agent: `_extract_codex_structured()` is fed the **raw** JSONL, not the rendered text. This means the raw JSONL still needs to reach the reconciliation step somehow.

   Resolved: write **both** streams. The rendered human-readable transcript goes to `log_file` (what `synlynk logs` shows). The raw JSONL is written unmodified to a sibling file, `log_file + ".jsonl"`, purely as the source `_extract_codex_structured()` reads from during reconciliation. This mirrors the existing `log_file + ".exit"` sibling-file convention already used for exit codes — no new pattern introduced, just one more sibling file for one agent.
   - `_reconcile_jobs()`'s Codex-specific token extraction reads `log_file + ".jsonl"` if it exists (falls back to `log_file` itself, unchanged, for every other agent and for Codex runs from before this change or where the sibling file is missing for any reason).
...
| Scenario | Result |
|---|---|
| Codex CLI version doesn't support `--json` (flag rejected, process exits nonzero immediately) | `.jsonl` sibling file is empty/missing → `_extract_codex_structured()` returns `None` → regex chain runs against `log_file` (which will contain Codex's stderr about the bad flag, not usable output, but this is no worse than today's behavior for a crashed job) |
...
- Integration test for `_render_codex_jsonl_to_log()`: feed a fixture JSONL byte stream through the renderer, assert the human-readable log output matches expected text, and assert the `.jsonl` sibling file contains the untouched raw input.
...
- **Placeholder scan:** none — every mechanism (adapter dispatch, usage-field mapping, sibling-file convention, fallback table) is fully specified with real code, not sketched.
- **Internal consistency:** the `.jsonl` sibling-file convention is introduced once (§3.3) and referenced consistently in both the rendering description and the reconciliation-read description; the fallback table (§3.4) covers every failure mode raised earlier in §3.2–3.3, nothing introduced there is left unhandled.
...
- **Ambiguity check:** "which `usage` object wins if multiple `turn.completed` events appear" is resolved explicitly (last one wins) even though not observed in testing, to avoid an undefined case later. The double-write (log + `.jsonl`) resolves what would otherwise be a genuine open question (raw JSONL needed for parsing vs. readable text needed for humans) rather than picking one and hand-waving the other.
