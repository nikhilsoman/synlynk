...
4. **Terminal/CLI Elitism:** Forcing all developers to install and learn standalone terminal CLI harness wrappers (`claude`, `agy`, `codex`, `grok`), ignoring the fact that the vast majority of modern developers work natively inside AI IDEs (Cursor, Windsurf, VS Code) or Desktop GUIs.

### The Synlynk v0.21.0 Thesis: Intent Transmission, Surface Agnosticism & First Real Win
Synlynk's purpose is to be the **synaptic link** between human architectural intent and multi-agent autonomous execution. Onboarding cannot simply be an inventory script—it must be an **interactive, progressive-disclosure onboarding journey** that moves the user from zero to their **First Real Autonomous Win** in under 5 minutes, meeting them on whichever surface they prefer (Web Browser, AI IDE, Desktop App, or Terminal).
...
│   • Agy & Claude Roles       • 1-Click GitHub Apps via      • Vizor 3-View Canvas (Tree,         │
│   • GitHub App Identities      Vizor (syn-*-qa[bot])          Tubemap, Application Screens)      │
│                                                                   │                              │
│                                                                   ▼                              │
...
- **What it ISN'T:**
  ...
  - *Not a Terminal-only CLI:* It is surface-agnostic, integrating natively into Cursor, Windsurf, VS Code, Claude Desktop, Antigravity IDE, and browser web interfaces.
...
```
  ...
  ┌──────────────────────────────────────────────────────────────────────────────────────────────────┐
  │                                                                                                  │
  │   SURFACE 1: WEB GUI (VIZOR)          SURFACE 2: AI IDEs & DESKTOP       SURFACE 3: HEADLESS CLI │
  │   (Zero-Terminal / Browser-First)     (Cursor, Windsurf, VS Code, Agy)   (Terminal Hackers & CI) │
  │   • http://localhost:27472            • Native Rules (.cursorrules/mdc)  • synlynk init / start  │
  │   • 3D Discovery Chips                • Model Context Protocol (MCP)     • synlynk dispatch      │
  │   • 3-View Canvas (Tree, Tube, UI)    • IDE AI acts as Home Conductor    • synlynk run --unattend│
  │   • 1-Click GitHub App Provisioning   • Reads .synlynk/context.md        • Headless Away Workers │
  │                                                                                                  │
  └──────────────────────────────────────────────────────────────────────────────────────────────────┘
```

### A. Surface Auto-Discovery
During initialization, Synlynk inspects the project and host environment:
1. **Cursor:** Detects `.cursor/` or running `Cursor.app` $ightarrow$ generates `.cursor/rules/synlynk.mdc`.
2. **Windsurf:** Detects `.windsurf/` $ightarrow$ generates `.windsurfrules`.
3. **VS Code / Copilot:** Detects `.vscode/` $ightarrow$ generates `.github/copilot-instructions.md` and `.vscode/extensions.json`.
4. **Claude Desktop:** Exposes Model Context Protocol (MCP) server via `synlynk mcp` configured in `claude_desktop_config.json`.
5. **Antigravity IDE:** Natively binds via `GEMINI.md` and Antigravity plugin tool definitions.
6. **Terminal CLI:** Drops `CLAUDE.md`, `AGENTS.md`, and `GROK.md` for terminal harness users.
...
| :--- | :--- | :--- | :--- |
| **Tier 1: Syntax & Schema Attestation** | Cursor (`.mdc`), Windsurf, Copilot, MCP | Automated Pytest in CI (`tests/test_instructions.py`, `tests/test_mcp.py`) | • Valid YAML frontmatter in `.cursor/rules/synlynk.mdc`<br>• Exact markdown syntax in `.windsurfrules` and `.github/copilot-instructions.md`<br>• Valid JSON-RPC 2.0 schemas for MCP tool definitions |
| **Tier 2: Prompt & Persona Emulation** | Claude, Gemini, GPT-4o | Headless LLM Evaluation Harness (`tests/test_persona_emulation.py`) | • Feed generated rule files + `.synlynk/context.md` to models in CI.<br>• Assert model identifies as Home Conductor.<br>• Assert model enforces Worktree-First policy.<br>• Assert model records decisions to `project-docs/memory.md`. |
...
| **Tier 4: Golden Fixture Repositories** | 4 Pre-Configured Testbeds (`fixture-cursor`, `fixture-windsurf`, `fixture-vscode`, `fixture-cli`) | `synlynk doctor --environment` in CI (`tests/test_environment_fixtures.py`) | • Test auto-detection of IDE markers (`.cursor/`, `.vscode/`, `.windsurf/`).<br>• Verify appropriate rules and MCP configurations are injected non-destructively without collisions. |
...
| :--- | :--- | :--- | :--- |
| **Workstream 1: Distribution & Cross-Environment Binding** | Installer & IDE Surface Generator | `install.sh`, `pipx`/`brew` packages, `.cursor/rules/`, `.windsurfrules`, `.github/copilot-instructions.md`, MCP server. | Codex |
