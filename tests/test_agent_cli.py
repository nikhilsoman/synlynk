...
def test_allow_distinct_qa_app_identities_to_submit_approving_pr_reviews(tmp_path, monkeypatch):
    ...
    with patch("subprocess.run", side_effect=[distinct_author, success_review, success_merge]) as mock_run, \
    ...
    with patch("subprocess.run", side_effect=[same_author, success_comment, success_merge]) as mock_run, \
         patch("synlynk.merge_oracle.require_merge_oracle", return_value={"merge_allowed": True}), \
         patch("synlynk.uxcore.rebase_pr_if_behind", return_value={"attempted": False}):
        result = uxcore.approve_pr(pr_number=1475)
        assert result.ok is True
    ...
    with patch("subprocess.run", side_effect=[distinct_author, integration_fail, success_comment, success_merge]) as mock_run, \
         patch("synlynk.merge_oracle.require_merge_oracle", return_value={"merge_allowed": True}), \
         patch("synlynk.uxcore.rebase_pr_if_behind", return_value={"attempted": False}):
        result = uxcore.approve_pr(pr_number=1475)
        assert result.ok is True
