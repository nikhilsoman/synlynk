...
def test_resolve_dispatch_gh_token_falls_back_to_synlynk_bot(tmp_path, monkeypatch):
    ...
    apps_dir = tmp_path / ".synlynk" / "github_apps"
    apps_dir.mkdir(parents=True)
    (apps_dir / "synlynk-bot.json").write_text(
        json.dumps(
            {
                "role": "synlynk-bot",
                "app_id": "9",
                "installation_id": "8",
                "private_key_path": "bot.pem",
            }
        )
    )
...
def test_resolve_dispatch_gh_token_returns_none_when_nothing_provisioned(
...
def test_dispatch_agent_does_not_inject_gh_token_by_default(tmp_path, monkeypatch):
    dispatch_mod, job, captured_env = _dispatch_with_fake_popen(
        ...
        monkeypatch,
        agent="codex",
        token_resolver=lambda role: (_ for _ in ()).throw(
            AssertionError("token resolver should not be called")
        ),
    )
