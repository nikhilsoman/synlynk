...
- **Vizor Navigation & Information Architecture Restructure (#1765):** Restructured Vizor into a two-tier native HTML5 accordion (`PERSONAL`/`TEAM`/`ENTERPRISE` → Workspaces → Category groups `STATUS`, `TOPOLOGIES`, `PROJECTIONS`, `TELEMETRY`). Sidenav utilizes native `<details name="...">` / `<summary>` tags with zero JavaScript framework dependencies. Added workspace Overview canvas (`overview.html`) as default view with sentinel alerts, stat cards, goal rollup, recent jobs, and category shortcuts, and cross-workspace Activity Stream (`activity.html`). Replaced top menu bar with location breadcrumbs and global corner actions. [@agy, @nikhilsoman]
