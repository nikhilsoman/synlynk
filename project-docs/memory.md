...
## Milestone v0.21.0 Architecture: Fleet Swarm Engine & Memory Compaction (decided 2026-09-12)
- **Approved Design Spec (`docs/superpowers/specs/2026-09-12-v0.21.0-swarm-engine-and-context-compaction-design.md`):**
  - **Cluster A (Ephemeral Swarm Infrastructure, #1341):** Pluggable `BaseRunnerDriver` interface with `LocalDockerDriver`, `KubernetesJobDriver`, and `FlyMachineDriver`. Implements `synlynk swarm dispatch --driver <D> --batch-size <N> --task "<T>"` streaming progress via `synlynk relay` with automated TTL self-destruction.
