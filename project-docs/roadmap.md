...
| **v0.10.1** | LIVE-1 Fixes: Agy + Grok Baseline Hardening | LIVE-1 root-cause fixes — Agy `--non-interactive` baseline wrong (caused 6h silent hang); Grok `--always-approve` in dispatch path (crashed CLI on every job) · `_check_job_stall()` inside `_reconcile_jobs()`: per-agent timeout, SIGKILL, `STALL_NO_OUTPUT` sentinel · `_preflight_dispatch()`: flag validation + network socket check → `HARNESS_PREFLIGHT_FAIL` sentinel · PR #82 · 503 tests | ✅ Shipped | Jul 2026 |
| **v0.11.0** | Harness Compatibility System (BS-14) | 5 new `state.db` tables (`harness_baselines`, `harness_records`, `harness_verb_map`, `harness_command_palette`, `harness_version_history`) · `synlynk probe`: version fingerprint → fast-path skip → upsert `harness_records` → palette scan → instruction fence write · `synlynk doctor` TC-1–TC-4 compliance suite (stdout, flags, network, verb map) · 64-row Command Interoperability Matrix across 5 verb categories × 4 agents · `_upsert_harness_fence()`: managed `<!-- synlynk:harness -->` section in CLAUDE.md / GEMINI.md / AGENTS.md · `HARNESS_VERSION_DRIFT` sentinel (1hr cache + live `--version` check in preflight) · `harness` + `model` fields in `.agents/<agent>.json` · PR #82 | ✅ Shipped | Jul 2026 |
