...
| **Repo/Workspace Visualization** | Three-view model: **Product view** (consumer-centric UX screen graph, static map of screens linking to each other) · **Logical view** (high-level component breakdown) · **Infra view** (network and cloud stack) — `synlynk viz` command or web companion | BS-6 | 📋 Brainstorm queued (story-f5513a93) | `synlynk workspace`, multi-repo intelligence, onboarding clarity |
