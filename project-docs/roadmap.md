...
| **v0.9.8** | Health Pulse + Lifecycle | `synlynk doctor` · registry-based `HealthCheck` · version nudge · onboarding completeness · agent profile gaps · identity key status · `synlynk exit` / `repair` / `sync` (OB-13–17) | 🔄 PR #70 open (2026-06-27) — 524 tests | Jul 2026 |
...
| **v0.10.1** | Harness Capability Awareness + Loop-Native Dispatch | `synlynk probe` — ambient daily capability drift detection; on-change: markdown commit + GitHub issue + analysis agent dispatch (implications + PR) · `dispatch_loop()` — composite termination (exit code + done_criteria + verify_cmd) · `/goal`-equivalent `## Dispatch Objective` injection · Stuck detection (exit repeat OR log pattern) → one-shot consult from capability matrix · Consult is advisory protocol — extensible to human team members in v1.0+ | 📋 Planned | Jul 2026 |
...
| **Skill Pack Interoperability + Benchmarks** | Prove synlynk enhances (not fights) skill packs (Superpowers, GStack, GSD, Everything-ClaudeCode) · 4-round benchmark: base → skill packs → synlynk → synlynk+skill packs · Token bloat reduction via context scoping · **AB-11 folded in: conflict taxonomy + `synlynk:start/end` instruction coexistence strategy** · Narrative: "coordination OS + domain expertise modules = better together" · Benchmark results as pre-launch asset | BS-7 | 📋 Brainstorm 2026-06-28/29 — benchmark execution week of 2026-06-30 | Dev preview launch narrative; AB-11 instruction coexistence (merged from BS-3); community/plugin ecosystem |
