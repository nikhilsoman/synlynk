...
| **Harness Compatibility** | Goal: synlynk equally effective regardless of entry harness · headless execution contract per agent (stdout flushing, PTY vs pipe, network deps) · per-agent dispatch flag map in `.agents/<agent>.json` · compliance test suite (scripted dispatch→execute→parse cycle for each harness) · `STALL_NO_OUTPUT` sentinel pattern · `synlynk doctor` flag validation · incident-driven: LIVE-1 (#81) — Agy silent hang + Grok `--always-approve` flag contamination | BS-14 | ⚡ **Next session** — brought forward from pre-GA; brainstorm agenda includes LIVE-1 findings, AB-0–10 adherence items, and cross-harness capability matrix | Cross-harness parity; FIX-1–4 from #81; v0.10.1 synlynk probe foundation |
