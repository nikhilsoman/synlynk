...
| **v0.10.0** | **Developer Preview** | **Scope: installable + daily driver.** P0: `pyproject.toml` + `pipx install git+<url>` · first-run polish (`synlynk help`, actionable errors) · README overhaul. P1: BS-12a agent role formalization · `synlynk release` Ship cycle stub (VERSION bump + CHANGELOG + blog stub) · `synlynk status` platform health (harness compliance + agent availability + budget pulse). P2: job completion summary (post-dispatch UX). **Not in scope:** `synlynk viz` (BS-6 needs brainstorm), BS-13 HUD (spec refresh required — see epic). Launch: HN + dev.to post after P0–P1. | 📋 Planned | Aug 2026 |
...
| **Workspace HUD** | Cycle-aware OS dashboard — all 6 cycles (Dream · Plan · Work · Ship · Maintain · Engage) light up based on detected activity; absorbs `synlynk status` platform health panel; job write path (`synlynk jobs register/complete`, #75/#76) is the Work cycle entry point; `synlynk watch` is the terminal surface; web board shares same read-only model. Original htop-style job list is the v1 of Work cycle only — full HUD is the expanded scope. **Spec refresh required before implementation.** | BS-13 | ⚠️ Spec refresh needed — original spec (`docs/superpowers/specs/2026-06-28-bs13-live-job-observatory-design.md`) covers Work cycle only; 6-cycle HUD + platform health panel not yet designed | `synlynk watch`, `synlynk viz`, multi-repo monitoring, `synlynk status` |
