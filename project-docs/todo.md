...
- [ ] **[OB-13] `synlynk exit` — clean departure, zero damage** — reverse all synlynk onboarding actions in the current repo and leave it unaware of synlynk protocols. Safe-first: `--dry-run` by default (prints what would be removed, does nothing). `--confirm` required to execute. What gets removed: `.synlynk/` directory, `CLAUDE.md`, `GEMINI.md`, `AGENTS.md`, `AI_INSTRUCTIONS.md`, `.cursorrules`, Cursor MDC files, Copilot ruleset, Windsurf config, `.agents/` directory. What is NOT removed: `project-docs/` (user-owned notes and devlogs — explicitly left behind with a note), `state.db` (capability history — user may want to export). Before removing anything, generate `synlynk-exit-handoff.md` at the repo root capturing: synlynk version at exit · mode · agent list + profile contents · active stories summary · cost totals · sentinel history · team members + last-activity dates · timestamp. Exit confirms: `"synlynk has been removed from this repo. Handoff doc saved to synlynk-exit-handoff.md."` The handoff doc is the only trace left — it is human-readable and machine-readable by `synlynk repair`. <!-- id:513 -->

- [ ] **[OB-14] `synlynk repair` — exit and rebuild from handoff** — two-phase command: (1) exit phase: if `.synlynk/` exists, run the equivalent of `synlynk exit --confirm` to produce a fresh handoff doc; (2) rebuild phase: read `synlynk-exit-handoff.md` (or `--handoff <path>`) and run `synlynk init` with the settings captured there — mode, agent list, org, repo, project_id — regenerating all instruction files and config from scratch. Restores `.agents/` profiles verbatim from the handoff doc. Prints a diff of what changed between the handoff state and the rebuilt state (e.g. new version, new default agent profiles). Use case: corrupted `.synlynk/` config, version upgrade with breaking init changes, migrating a repo to a new machine, or recovering after an accidental `rm -rf .synlynk/`. Does NOT restore `project-docs/` (assumed still present — if missing, prints a warning and suggests `synlynk init`). <!-- id:514 -->
...
- [ ] **[AB-11] Agent-encounters-synlynk behaviour study — conflict patterns taxonomy** — systematic study of what each agent (Claude, Agy/Gemini, Codex) does when it first encounters synlynk in a repo, specifically when pre-existing project conventions conflict with synlynk's generated SOP. Seed case: Agy in rxcc (2026-06-24) — 6 observed conflicts: (1) identity mismatch: synlynk maps GEMINI.md → `agy-2.x` slot but repo uses `Gemini 2.5 Pro`; (2) commit trailer: `AGY <noreply@antigravity.dev>` vs `Gemini 2.5 Pro <noreply@google.com>`; (3) branch naming: `feat/agy/` vs `feat/gemini/`; (4) workflow: worktree-first vs standard checkout; (5) docs location: `project-docs/` vs root; (6) session protocol: 3-row vs 2-row greeting. Agy's response: correctly detected all 6, ran `git checkout` to undo synlynk's instruction update, declared the pre-existing GEMINI.md as "source of truth." **This is correct safety-first behaviour, not defiance** — Agy's authority hierarchy ranked existing project instructions above external tool injection. Synlynk lost the authority contest because `synlynk instructions update` had no markers to update (no `<!-- synlynk:start -->`), so it overwrote the entire file, which Agy read as a hostile takeover. Study goals: (a) reproduce across Claude and Codex to map per-agent resistance thresholds; (b) classify conflict types — identity, convention, location, protocol, authority; (c) document which conflicts are synlynk's fault (missing markers, wrong identity mapping) vs genuine design tension. Outcome: `docs/superpowers/specs/agent-encounter-conflict-taxonomy.md` <!-- id:411 -->

- [ ] **[AB-12] Instruction coexistence strategy — how synlynk should handle pre-existing conventions** — design and implement synlynk's approach to repos with established conventions that differ from synlynk defaults. Three failure modes to address: (a) **Missing markers** — `synlynk instructions update` on a file with no `<!-- synlynk:start -->` markers overwrites the entire file. Fix: detect marker absence, offer `--merge` mode that appends synlynk section without touching existing content; (b) **Identity mismatch** — synlynk hard-codes `agy` as the GEMINI.md agent name. Fix: make agent identity configurable per project in `.synlynk/config.json` — `agent_identity.agy_name`, `agent_identity.agy_email`, `agent_identity.claude_name`, etc. Allow `synlynk init --agy-identity "Gemini 2.5 Pro" --agy-email "noreply@google.com"`; (c) **Convention conflict** — synlynk's generated instructions assume `project-docs/` layout, worktree-first workflow, specific branch naming. Fix: `synlynk init` should scan for existing conventions (git log for branch naming patterns, README/CONTRIBUTING.md for commit format, existing docs structure) and generate instructions that respect them rather than imposing defaults. Add `synlynk instructions conflict-check` that explicitly surfaces conflicts between synlynk's injected blocks and detected project conventions before applying. Outcome: `synlynk instructions update --merge` flag, configurable agent identity, convention-aware init. <!-- id:412 -->

- [ ] **[AB-13] Cross-agent authority hierarchy — framing synlynk as project protocol not tool instruction** — investigate why Agy's authority hierarchy ranked existing project instructions above synlynk's injected SOP, and how to reframe synlynk instructions so agents treat them as project-owned rather than tool-owned. Hypothesis: agents are trained to be cautious about third-party tools overwriting project files, but highly deferential to project-native conventions. Synlynk's current framing ("synlynk: The Universal Context Switchboard") positions it as an external tool. Alternative: reframe the injected sections as "Project AI Protocol" — authored by the project, maintained by synlynk, not owned by synlynk. Proposed change: replace `<!-- synlynk:start -->` header text from "synlynk-managed section" to "Project AI Protocol — maintained via synlynk" in all instruction files. Study whether this framing reduces resistance across agents. Measure: run the Agy rxcc conflict scenario with reframed markers and compare behaviour. Also study: does `synlynk exec` framing (injecting context.md before session) face the same authority problem? Does the agent treat `context.md` as trusted project state or as external tool injection? Outcome: revised instruction file templates + `docs/superpowers/specs/synlynk-authority-framing-study.md` <!-- id:413 -->

- [ ] **[AB-14] AB series tests** — reproduce Agy rxcc conflict scenario in a tmp_path fixture: pre-existing GEMINI.md without markers, `synlynk instructions update` called, verify `--merge` appends rather than overwrites; verify conflict-check output lists all 6 detected conflicts; verify configurable agent identity flows through to generated commit trailer text; verify reframed markers text in all instruction templates. <!-- id:414 -->
...
- [ ] **[CA-1] 9-dimension audit per command** — for every command, document: (1) when it makes sense to use, (2) hook/trigger conditions (git hooks, file-watch, CI, daemon schedule, manual), (3) pre-requisites (.synlynk/ present, AI CLI on PATH, daemon running, etc.), (4) input context consumed (context.md sections, SQLite tables, env vars), (5) skills/agents it can invoke, (6) output contract (stdout format, files written, DB rows changed, exit codes), (7) whether it can trigger an agent/autopilot downstream, (8) discoverability surface (--help text, context.md injection, site docs), (9) error UX when pre-reqs unmet. **For `dispatch` specifically:** dimension 4 audit must answer: is the injected context task-scoped or full-project? Is it agent-scoped? Is `context.md` shared across concurrent dispatches (race condition)? Observed failure: 119KB full-project context injected for a cross-repo PR review task (2026-06-24). Outcome: `docs/superpowers/specs/command-audit-matrix.md` <!-- id:311 -->
...
- [ ] **[DevX-1] Context-layer brainstorm for v0.9.4** — Run the brainstorm session from spec `docs/superpowers/specs/2026-06-24-v0.9.4-context-layer-design-question.md`. Key decisions: hybrid read in `generate_context()` (SQLite tasks + file memory), relay wire protocol (structured events, not markdown blob), `context.md` as backwards-compat view. **Agenda expanded 2026-06-24** — concrete failure mode observed: `synlynk dispatch agy` on a cross-repo PR review injected 119KB of synlynk project-docs context; Agy received ~300× more tokens than needed (task was self-contained in `--task` string). New brainstorm agenda items: (Q6) task-scoped context via `generate_context(scope="task")`; (Q7) cross-repo dispatch — should project-docs be suppressed entirely?; (Q8) per-dispatch context file at `.synlynk/jobs/<job_id>/context.md` to fix concurrent dispatch race; (Q9) context size budget enforcement in dispatch; (Q10) per-agent context profiles (Agy code-review profile vs Claude architecture profile). See updated spec for full question set. <!-- id:300 -->
...
- [ ] **[DevX-3] Dispatch lifecycle observability** — `synlynk jobs` one-shot table (v0.9.4) + `--watch` stream; context size visible in dispatch header line. Pre-flight gate (daemon→agent→budget), per-lifecycle relay events. HUD (TUI jobs board, curses-free): **v1.0**. <!-- id:302 -->
...
- [ ] **HUD** — TUI jobs board (jobs · stories · budget pulse · relay status) built on `daemon_jobs` + SQLite; `synlynk jobs` one-shot is the data layer prototype (v0.9.4); HUD is the rendering layer. curses-free (rich or textual). <!-- id:270 -->
