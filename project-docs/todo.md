...
- [ ] v0.10.0 (P0): `synlynk init --wizard` — FTUE wizard: mandatory Phase 0 silent scan, 6 screens (home harness · workspace topology · skills · agent fleet · roles · launch cheat sheet) + multi-repo sub-flow (2ab combined name+picker → 2c confirm); pure stdlib TUI (termios); writes workspace config + state.db + agent directive role blocks on final screen; Ctrl-C before completion leaves no state; spec: `docs/superpowers/specs/2026-07-01-bs17-ftue-onboarding-design.md` [ux] <!-- id:story-v010-wizard -->
- [ ] v0.10.0 (P0): `synlynk scan` — re-runnable repo analysis command: detects topology (single/mono/multi), fingerprints stack per repo/package (14 file-presence heuristics), parses README+CLAUDE.md+GEMINI.md+AGENTS.md, maps to workspace in state.db, regenerates structured context.md; flags: `--refresh` `--add <path>` `--remove <path>` `--dry-run`; called internally by wizard Phase 0 and available standalone; spec: `docs/superpowers/specs/2026-07-01-bs17-ftue-onboarding-design.md` [platform] <!-- id:story-v010-scan -->
