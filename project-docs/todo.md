...
- [ ] v0.10.0 (P0): pyproject.toml packaging + pipx install — `pyproject.toml` with `[project]` + `[project.scripts]`, `pipx install git+<url>` flow, `synlynk upgrade` aware of pipx vs manual install, VERSION single source of truth [packaging] <!-- id:story-v010-packaging -->
