...
| Light (~$1) | 0 | $1 | $0 |
| Medium (~$2–3) | 0 | $2.50 | $0 |
| Heavy / brainstorm-driven (~$4–6) | 7 | $4.70 | $32.68 |
| **Total to date** | | | **~$32.68** |
...
| 2026-08-09/10 | Phase 0 (agent-roles-charters-design, terminology-only) executed via subagent-driven-development: 5 tasks dispatched to Codex/Agy per role split, PR #880 opened. "Dispatch for review" hit three straight failures before succeeding — Grok 402 (Build usage balance exhausted, new failure mode), Agy silently no-op'd under `--requires-gh-write` (exit 0, zero real work, new bug documented), then a real `GEMINI.md` merge conflict against `origin/main` (concurrent PR #874 regenerating the same harness fence line) resolved in a race between Claude's manual fix and Agy's own dispatched job — Agy's push won and was verified as the legitimate merged state. Housekeeping pass caught a missing Blog Post Protocol entry for #880 (retroactive post #113 + PR #885, itself reviewed/merged non-author), a cost-capture gap (this row), and 3 memory writes (Grok's second failure mode, the `--requires-gh-write` no-op, a cross-reference lead on the still-open GEMINI.md #884 investigation). | ~150K | ~60K | ~600K | ~$0.45 | ~$0.90 | ~$4.50 |
